...
assert.ok(feedbackMenu.includes('id="clear-resolved-comments"'), "feedback menu must expose Clear resolved");
...
assert.equal((feedbackMenu.match(/role="menuitem"/g) || []).length, 4, "feedback menu must contain only Save, Clear resolved, Clear all comments and Canvas info");
...
assert.ok(boardSource.includes("window.__SUPERCANVAS_SERVER__"), "Save feedback must write through the review server when the canvas is served");
assert.ok(boardSource.includes('markSaved(payload)'), "every save path must mark the draft as submitted");
...
assert.ok(boardSource.includes("window.showSaveFilePicker"), "a canvas opened from disk must keep a manual save path");
assert.ok(boardSource.includes('qs("#canvas-info-btn").onclick'), "Canvas info menu item must open the info modal");
assert.ok(boardSource.includes("feedbackProtocol.rotate(feedbackMeta.archive, reviewCycle, loadComments().concat(carriedArchive(draftEnvelope())))"), "Clear resolved must rotate through the protocol and carry comments archived by an earlier save");
const savePayloadBody = boardSource.match(/function savePayload\(\) \{[\s\S]*?\n {2}\}/)?.[0];
assert.ok(savePayloadBody, "the board runtime must build its save payload in savePayload()");
assert.equal(savePayloadBody.includes("rotate"), false, "a plain save must not archive resolved comments before the reviewer clears them");
assert.ok(boardSource.includes('comment.target.type !== "canvas"'), "unanchored legacy Canvas comments must not render pins");
assert.ok(boardSource.includes('if (!isReviewTarget(comment)) return;'), "only anchored comments may render pins");
assert.ok(boardSource.includes('var label = resolved ? "✓"'), "a resolved comment must keep a pin so the reviewer can read the change summary before clearing it");
